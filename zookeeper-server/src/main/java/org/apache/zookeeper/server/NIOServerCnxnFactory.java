...
public class NIOServerCnxnFactory extends ServerCnxnFactory {
    ...
    /**
     * 接受客户端连接线程【accept连接并轮训注册到selector线程处理读写事件】
    ...
    public class SelectorThread extends AbstractSelectThread {
        ...
        /**
         * select注册到selector上的连接、检测读写事件并处理监听变更与处理注册新accept连接
         */
        private void select() {
            try {
                selector.select();
    ...
    /**
     * 开启accept线程、io线程、业务work线程、超时检测线程
     */
    @Override
    public void start() {
        stopped = false;
